...
class EmailHelper
{
    ...
    public function sendOfficialCredentials(string $toEmail, string $name, string $jsca_id, string $password, string $type): bool
    {
        $this->mailer->setTo($toEmail);
        $this->mailer->setSubject('JSCA — Your Official Account Credentials');
        $this->mailer->setMessage($this->officialCredentialsTemplate($name, $jsca_id, $password, $type));
        return $this->mailer->send(false);
    }
    ...
    private function officialCredentialsTemplate(string $name, string $jsca_id, string $password, string $type): string
    {
        $loginUrl = base_url('login');
        return <<<HTML
        <div style="font-family:Segoe UI,sans-serif;max-width:480px;margin:auto;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
          <div style="background:#1a3a5c;padding:20px 24px;">
            <h2 style="color:#fff;margin:0;font-size:18px;">&#127951; JSCA ERP &mdash; Official Account Created</h2>
          </div>
          <div style="padding:24px;">
            <p style="color:#333;">Dear <strong>{$name}</strong>,</p>
            <p>You have been registered as a <strong>{$type}</strong> in the JSCA system. Here are your login credentials:</p>
            <table style="width:100%;border-collapse:collapse;margin:16px 0;">
              <tr><td style="padding:8px;background:#f8f9fa;font-weight:600;width:40%;">JSCA Official ID</td><td style="padding:8px;background:#f8f9fa;">{$jsca_id}</td></tr>
              <tr><td style="padding:8px;font-weight:600;">Password</td><td style="padding:8px;">{$password}</td></tr>
            </table>
            <p style="color:#e74c3c;font-size:12px;"><strong>Please change your password after first login.</strong></p>
            <a href="{$loginUrl}" style="display:inline-block;background:#1a3a5c;color:#fff;padding:10px 24px;border-radius:6px;text-decoration:none;margin-top:8px;">Login to JSCA ERP</a>
